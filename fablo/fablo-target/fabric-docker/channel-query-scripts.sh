...
channelQuery() {
  if [ "$#" -eq 1 ]; then
    ...
    [ "$1" = "list" ] && [ "$2" = "rektor" ] && [ "$3" = "peer0" ]
  then

    peerChannelListTls "cli.rektor.itk.ac.id" "peer0.rektor.itk.ac.id:7061" "crypto-orderer/tlsca.orderer.itk.ac.id-cert.pem"
  ...
  then
    ...
    [ "$1" = "getinfo" ] && [ "$2" = "ijazah-channel" ] && [ "$3" = "rektor" ] && [ "$4" = "peer0" ]
  then

    peerChannelGetInfoTls "ijazah-channel" "cli.rektor.itk.ac.id" "peer0.rektor.itk.ac.id:7061" "crypto-orderer/tlsca.orderer.itk.ac.id-cert.pem"

  elif [ "$1" = "fetch" ] && [ "$2" = "config" ] && [ "$3" = "ijazah-channel" ] && [ "$4" = "rektor" ] && [ "$5" = "peer0" ]; then
    TARGET_FILE=${6:-"$channel-config.json"}

    peerChannelFetchConfigTls "ijazah-channel" "cli.rektor.itk.ac.id" "$TARGET_FILE" "peer0.rektor.itk.ac.id:7061" "crypto-orderer/tlsca.orderer.itk.ac.id-cert.pem"

  elif [ "$1" = "fetch" ] && [ "$3" = "ijazah-channel" ] && [ "$4" = "rektor" ] && [ "$5" = "peer0" ]; then
    BLOCK_NAME=$2
    TARGET_FILE=${6:-"$BLOCK_NAME.block"}

    peerChannelFetchBlockTls "ijazah-channel" "cli.rektor.itk.ac.id" "${BLOCK_NAME}" "peer0.rektor.itk.ac.id:7061" "crypto-orderer/tlsca.orderer.itk.ac.id-cert.pem" "$TARGET_FILE"
...
printChannelsHelp() {
  ...
  echo "fablo channel list rektor peer0"
  echo -e "\t List channels on 'peer0' of 'Rektor'".
  echo ""
  ...
  echo "fablo channel getinfo ijazah-channel rektor peer0"
  echo -e "\t Get channel info on 'peer0' of 'Rektor'".
  echo ""
  echo "fablo channel fetch config ijazah-channel rektor peer0 [file-name.json]"
  echo -e "\t Download latest config block and save it. Uses first peer 'peer0' of 'Rektor'".
  echo ""
  echo "fablo channel fetch <newest|oldest|block-number> ijazah-channel rektor peer0 [file name]"
  echo -e "\t Fetch a block with given number and save it. Uses first peer 'peer0' of 'Rektor'".
  echo ""
